...
def save_quiz_start_data():
    ...
    for user in quiz_start_count:
        ...
        #plt.show()
        plt.savefig("C:/Users/shubh/PycharmProjects/User_Engagement/quiz_start_plot/"+usr+".png")
        plt.clf()
    for user in quiz_start_count:
        index = quiz_start_count[user].keys()
        count = quiz_start_count[user].values()
        plt.plot(index, count)
    plt.savefig("C:/Users/shubh/PycharmProjects/User_Engagement/source/Data_Extraction/quiz_data_extract/Quiz_start_plot.png");
    plt.show();
