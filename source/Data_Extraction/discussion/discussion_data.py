...
def save_discussion_data():
    ...
    for user in discussion_clicks:
        ...
        #plt.show()
        plt.savefig("C:/Users/shubh/PycharmProjects/User_Engagement/discussion_plot/"+usr+".png")
        plt.clf()
    for user in discussion_clicks:
        index = discussion_clicks[user].keys()
        count = discussion_clicks[user].values()
        plt.plot(index, count)
    plt.savefig("C:/Users/shubh/PycharmProjects/User_Engagement/source/Data_Extraction/discussion/discussion_plot.png");
    plt.show();
